class ClinicsController < ApplicationController
  ...
  # GET /clinics/district/:district
  def district
    @clinics = Clinic.where("district = ?", params[:district])

    respond_to do |format|
      if ok
        format.html { redirect_to clinics_url, notice: 'Clinic was found.' }
        format.json { render json: @clinics }
      else
        format.html { notice:"No clinic" }
        format.json { render json: @clinics.errors, status: :unprocessable_entity }
      end
    end
  end
  # GET /clinics/new
  ...
  private
    ...
    def clinic_params
      params.require(:clinic).permit(:name, :street, :parish, :county, :district)
    end
end
